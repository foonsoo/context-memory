...
The Decision Brief regression is runnable with `PYTHONPATH=src python3 benchmarks/run_decision_evaluation.py`. Its frozen schema-v1 fixture covers changed decisions, conflicting evidence, rejected alternatives, outcome feedback, missing rationale, stale external sources, negative questions, lexical paraphrases, duplicate summaries, cross-project isolation, and a 120-distractor candidate set. Report schema v3 evaluates both FTS-only and default local-hash modes and records Recall@5, MRR@5, negative-query false-result rate, duplicate rate, current-decision accuracy, stale-decision leakage, unsupported-claim rate, source recovery, useful-history recall, compact JSON character size, p50/p95 latency, and per-scenario retrieval-gate reasons and components. The separate embedding evaluation now uses gate-aware report schema v3 and freezes partial-wording, typographical, abbreviation, and Korean expression variations alongside negative questions. These deterministic suites are exit-gate regressions, not evidence that the product improves human decisions.

The completed P2.5 exit matrix is preserved in `benchmarks/results/p25-exit-2026-08-17.json`. It compares the final default path with frozen baseline commit `aa4d49c`, records the documented context-size tolerance, and includes unit, compile, installed-wheel, official MCP SDK, embedding, Decision Brief, and discovery checks. Latency values are environment-specific; accuracy and safety assertions come from deterministic fixtures.

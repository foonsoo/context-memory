...
1. **Freeze the compatibility baseline.** Snapshot the MCP tool catalog and schemas, CLI help/command surface, export/import format, compact context and Decision Brief fixtures, Wiki Markdown output, and benchmark thresholds before moving code. Keep style-only, structural, and behavioral changes in separate commits.
2. **Adopt PEP 8 as the repository convention.** Add a development-only formatter/linter configuration and CI checks for imports, whitespace, naming, compound statements, and dead imports. Use four-space indentation, one statement per line, descriptive names, a 79-character code limit, and a 72-character comment/docstring limit. Allow only narrow, documented exceptions for unbreakable literals or generated data. Clear violations module by module instead of hiding the current baseline behind a permanent ignore list.
3. **Remove proven dead or redundant code first.** Build a public/private call inventory across MCP, CLI, hooks, tests, and documented imports. Remove a private helper or compatibility branch only when characterization tests prove it is unreachable or a documented deprecation path exists. Do not treat shorter code as inherently better.
4. **Extract pure domain helpers from `store.py`.** Move constants, validation, retrieval scoring/gates, Wiki rendering/lint calculations, checkpoint policy calculations, and audit serialization into focused modules with no database ownership. Keep transaction boundaries and SQL in `MemoryStore` until each extraction has direct tests.
...
8. **Verify every slice.** Require the full Python 3.11–3.14 matrix, installed-wheel/restart test, official MCP SDK interoperability test, `compileall`, frozen Decision/embedding/discovery evaluations, export/import round trip, and `git diff --check`. Compare query counts and p50/p95 retrieval latency before accepting a structural change.

...
class MemoryStore:
    ...
    def create_investigation(
        ...
        request.pop("self")
        request.pop("idempotency_key")
        if hit := self._idem("create_investigation", idempotency_key, request):
            return hit
        if not all(
            value.strip()
    ...
    def record_source_analysis(
        ...
        if existing:
            chain = self.get_investigation(investigation_id, existing["id"])[
                "source_analyses"
            ][0]
            repeated_claims = []
    ...
    def complete_investigation(self, investigation_id: str) -> dict[str, Any]:
        ...
        if investigation["status"] == "completed":
            return self.get_investigation(investigation_id)["investigation"]
        completed_at = now()
        with self.tx() as cx:
            cx.execute(
    ...
    def create_wiki_page(
        ...
        if hit := self._idem("create_wiki_page", idempotency_key, request):
            return hit
    ...
    def export_wiki_markdown(
        ...
        for index, page in enumerate(pages):
            ...
            body = self.render_wiki_revision(current["id"])[
                "markdown"
            ].splitlines()
            metadata = [
                "---",
    ...
    def create_checkpoint(
        ...
        if hit := self._idem("create_checkpoint", idempotency_key, request):
            return hit
            ...
            for field in ("command", "details"):
                ...
                if value is not None:
                    if not isinstance(value, str) or not value.strip():
                        raise ValueError(
                            f"test result {field} cannot be empty"
                        )
        ...
        with self.tx() as cx:
            if existing:
                ...
                if any(
                    ...
                    self._stale_wiki_revisions_for_memory(
                        cx, mid, "cited memory materially updated"
                    )
            else:
                cx.execute(
                    """INSERT INTO memories(id,project_id,scope_id,type,status,title,content,confidence,importance,valid_from,valid_until,
            ...
            for eid in source_event_ids or []:
                ...
                if not event or event["project_id"] != project_id:
                    raise ValueError(f"invalid source event: {eid}")
                cx.execute(
            ...
            self._save_idem(
                cx, "upsert_memory", idempotency_key, request, item
            )
        return item
    ...
    def _retrieval_gate(candidates: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        runner_similarity = (
            runner.get("semantic_similarity") if runner else None
        )
        semantic_separation = (
        ...
        if (
            similarity is None
            or similarity < NEGATIVE_VECTOR_ONLY_MIN_SIMILARITY
        ):
            return {
                "status": "no_confident_match",
    ...
    def export_audit_chain(self, project_id: str) -> dict[str, Any]:
        ...
        return {
            "format": "context-memory-audit-chain",
            "version": 1,
            "project_id": project_id,
    ...
    def verify_audit_chain(
        ...
        errors: list[str] = []
        if (
            bundle.get("format") != "context-memory-audit-chain"
        ...
        for index, checkpoint in enumerate(checkpoints):
            ...
            if not all(
                ...
                errors.append(
                    f"{label} range and entry_count must be integers"
                )

...
5. **Split persistence by bounded domain behind a stable facade — in progress.** Composed repositories now own bounded project/session/evidence, memory lifecycle and embedding persistence, ranked retrieval candidate queries, investigation read-model, Wiki identity, checkpoint observation, maintenance execution/audit queries, and portable project transfer using the facade's transaction connection. `context_memory.store.MemoryStore` remains the compatibility facade without mixin inheritance or repository-to-repository dependencies. Continue splitting context and decision assemblies until `store.py` owns orchestration and transaction boundaries rather than feature SQL.

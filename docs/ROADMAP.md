...
P0 Decision Brief/evaluation, P1 Research-to-Decision provenance, P2 topic Wiki revisions, P2.5 non-neural retrieval quality/latency, P3 Decision Wiki lint/review, P4 source revalidation, and the P5 navigation/export contracts are implemented. The active runtime was backed up and upgraded through Wiki migrations 13–16 on 2026-08-20. The first live Wiki review cycle produced actionable contract evidence: task handoffs inflated omission lint and task-only generation failures lacked diagnostics. Those issues are corrected without adding a UI. The next evidence-led step is additional live review cycles to validate whether mixed queue ordering and client-restart ergonomics repeat often enough to justify contract or operational changes. Neural embedding remains an optional evaluated adapter and is not part of the active implementation path because its measured latency cost did not justify making it the default.

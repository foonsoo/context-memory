...
FTS5 BM25 supplies lexical relevance. A dependency-free on-device feature-hash projection is enabled by default and improves fuzzy, morphological, and partial-phrase recall without claiming neural semantic understanding. Lexical and local-similarity ranks are combined with reciprocal-rank fusion, then receive small bounded adjustments for importance, confidence, confirmation freshness, and explicit helpful/incorrect feedback. Search results expose each weighted score component, the component ranks, and similarity so ranking remains inspectable. Expired validity windows are filtered at query time. `get_context` excludes proposed and superseded entries by default, includes disputed entries as warnings, respects optional scope, and greedily selects complete blocks within a strict character budget. Each block cites source event IDs.

`decision_context` is a read-only projection over `get_context`, not a second index or recommendation engine. Its versioned Decision Brief groups cited memories by explicit lifecycle, type, and tags, preserves a chronological decision history, labels evidence gaps and unreviewed memories as uncertainty, and returns no recommendation. It reranks only the already bounded and budgeted context items for decision intent, lifecycle/type, direct provenance, decision roles, and unsupported/stale/handoff penalties; general search remains unchanged. From at most three current-decision seeds it may then add one-hop supports, depends-on, supersession, or same-investigation evidence, bounded to 50 candidates and the existing item/character budgets. The embedded compact retrieval result exposes the base rank, inferred roles, every rerank score component, expansion path and limit, and the exact budget behavior used to build the brief.

Research claims form an append-only chain within an investigation. A claim may cite earlier claims from the same source analysis by key or claims from a prior analysis by the explicit `(source_analysis_id, claim_key)` pair; references cannot cross investigation boundaries. Decisions may record an expected outcome, and later outcome claims may classify their effect as confirming, weakening, disputing, or superseding the cited decision. `decision_context` projects those linked claims as cited expected-versus-observed comparisons without changing either memory's lifecycle state.

`topic-wiki/v1` is a durable projection layer, not a second knowledge authority. Topic pages hold identity and explicitly manual notes; immutable generated revisions snapshot the standard Decision Brief sections and exact memory/event citation pairs. Revisions move from `proposed` to `published` or `rejected`; a published revision becomes `stale` when replaced or when a cited memory is materially updated, superseded, disputed, expired, or rejected. Markdown is rendered from SQLite on demand, so it remains a portable view rather than writable state.

`research-provenance/v1` adds an append-only source-analysis chain without making external documents authoritative. An investigation stores intent and lifecycle; each source analysis is deduplicated by stable identity plus version or analyzed-content fingerprint. A single transaction records typed claims as immutable events, cited memories, and exact causal links. Inference is structurally prevented from becoming active during ingestion. Source analyses and claim/link rows are append-only, and export/import preserves the complete chain.

The default local projection stores only rebuildable vectors in `memory_embeddings`; events and memories remain authoritative. Set `CONTEXT_MEMORY_EMBEDDINGS=off` for explicit FTS-only operation, or `local-hash` to state the default explicitly. `memory_feedback` records retrieved, used, helpful, and incorrect signals in a disposable usage projection. Memories separately retain `observed_at` and `last_confirmed_at` so personal ranking can decay stale assumptions without rewriting evidence.
...
Project-owned search aliases expand known domain vocabulary before FTS matching. Aliases are explicit data, not inferred facts. Lexical retrieval first requires every query-term group, treating aliases as deterministic alternatives within a group; when that strict pass cannot fill the requested count, it falls back to the broader expanded `OR` query and reports the selected strategy. `graph_traverse` walks verified memory edges with a depth limit of five and filters out non-current nodes by default. Both aliases and edges are exportable projections; neither replaces source events.

...
def encrypt_file(
    ...
    aes_gcm_type, scrypt_type = _crypto()
    ...
    key = scrypt_type(
        salt=salt, length=32, n=header["n"], r=header["r"], p=header["p"]
    ).derive(passphrase.encode())
    ciphertext = aes_gcm_type(key).encrypt(
        nonce, source.read_bytes(), MAGIC + encoded
    )
    destination.write_bytes(
        MAGIC + struct.pack(">I", len(encoded)) + encoded + ciphertext
    )
    return {
        "encrypted": True,
        "envelope_version": 1,
...
def decrypt_file(
    ...
    try:
        ...
        plaintext = aes_gcm_type(key).decrypt(
            nonce, payload[offset + size :], MAGIC + encoded
        )

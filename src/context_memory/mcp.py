...
TOOLS = [
    ...
    {"name": "record_event", "description": f"Append immutable raw evidence. Session events with these kinds are eligible for automatic proposed-memory extraction: {promotable_kinds_text()}. Other kinds, including message, remain valid immutable events but are not automatically promoted; the response includes a non-fatal promotion advisory. Success is returned only after durable DB commit.", "inputSchema": obj({"project_id":{"type":"string"},"kind":{"type":"string"},"content":{"type":"string"},"session_id":{"type":"string"},"scope_id":{"type":"string"},"source_uri":{"type":"string"},"metadata":{"type":"object"},"idempotency_key":{"type":"string"}}, ["project_id","kind","content"]), "annotations":{"readOnlyHint":False}},
    ...
    {"name":"event_poll","description":"Poll an exact event stream from a durable per-consumer acknowledged cursor. Delivery does not advance acknowledgement.","inputSchema":obj({"project_id":{"type":"string"},"consumer_id":{"type":"string"},"kinds":{"type":"array","items":{"type":"string"}},"scope_id":{"type":"string"},"limit":{"type":"integer","minimum":1,"maximum":1000}},["project_id","consumer_id"]),"annotations":{"readOnlyHint":False}},
    {"name":"event_ack","description":"Monotonically acknowledge a cursor previously delivered by event_poll for the same consumer and stream definition.","inputSchema":obj({"project_id":{"type":"string"},"consumer_id":{"type":"string"},"cursor":{"type":"integer","minimum":0},"kinds":{"type":"array","items":{"type":"string"}},"scope_id":{"type":"string"}},["project_id","consumer_id","cursor"]),"annotations":{"readOnlyHint":False}},
    ...
    {"name":"review_queue","description":"List proposed memory candidates and latest Decision Wiki revisions with deterministic lint findings in one explicit review queue. Actionable proposed Wiki revisions are ordered before accumulated memory candidates; queue_priority and created_at make the deterministic order inspectable. Memory candidates use review_action; proposed Wiki revisions expose approve/reject routes through wiki_revision_transition.","inputSchema":obj({"project_id":{"type":"string"}},["project_id"]),"annotations":{"readOnlyHint":True}},
    ...
    {"name":"decision_context","description":"Compose a versioned, cited Decision Brief from existing retrieval without generating a recommendation or changing memory state.","inputSchema":obj({"project_id":{"type":"string"},"question":{"type":"string"},"char_budget":{"type":"integer","minimum":0,"maximum":100000},"scope_id":{"type":"string"},"discover_projects":{"type":"boolean"}},["project_id","question"]),"annotations":{"readOnlyHint":True}},
    {"name":"investigation_create","description":"Create selective research intent for a decision under research-provenance/v1.","inputSchema":obj({"project_id":{"type":"string"},"question":{"type":"string"},"reason":{"type":"string"},"decision_to_inform":{"type":"string"},"constraints":{"type":"array","items":{"type":"string"}},"initiator":{"type":"string"},"scope_id":{"type":"string"},"investigation_id":{"type":"string"},"idempotency_key":{"type":"string"}},["project_id","question","reason","decision_to_inform"]),"annotations":{"readOnlyHint":False}},
    {"name":"investigation_record_source","description":"Atomically record one identified source version and consequential evidence, inference, action, decision, rationale, or outcome claims. Claims may cite earlier claims in this analysis or explicit claims from prior analyses. Inferences always remain proposed.","inputSchema":obj({"investigation_id":{"type":"string"},"source":obj({"source_type":{"type":"string"},"stable_source_id":{"type":"string"},"canonical_uri":{"type":"string"},"source_version":{"type":"string"},"source_updated_at":{"type":"string"},"retrieved_at":{"type":"string"},"section_anchor":{"type":"string"},"access_reason":{"type":"string"},"analysis_method":{"type":"string"},"content_fingerprint":{"type":"string"}},["source_type","stable_source_id","access_reason","analysis_method"]),"claims":{"type":"array","items":obj({"key":{"type":"string"},"role":{"type":"string","enum":["evidence","inference","action","decision","rationale","outcome"]},"title":{"type":"string"},"content":{"type":"string"},"evidence_claim_keys":{"type":"array","items":{"type":"string"}},"evidence_claim_refs":{"type":"array","items":obj({"source_analysis_id":{"type":"string"},"claim_key":{"type":"string"}},["source_analysis_id","claim_key"])},"expected_outcome":{"type":"string"},"outcome_effect":{"type":"string","enum":["confirms","weakens","disputes","supersedes"]},"memory_type":{"type":"string","enum":["fact","decision","preference","constraint","procedure","summary","task","other"]},"memory_status":{"type":"string","enum":["proposed","active"]},"confidence":{"type":"number","minimum":0,"maximum":1},"importance":{"type":"number","minimum":0,"maximum":1}},["key","role","content"])},"session_id":{"type":"string"},"idempotency_key":{"type":"string"}},["investigation_id","source","claims"]),"annotations":{"readOnlyHint":False}},
    {"name":"investigation_get","description":"Read an investigation intent, source identities and versions, typed claims, memories, events, and causal evidence links.","inputSchema":obj({"investigation_id":{"type":"string"}},["investigation_id"]),"annotations":{"readOnlyHint":True}},
    {"name":"source_reinspection_request","description":"Request client-owned reinspection of one recorded external source because it is old, unavailable, or known to have a newer version. The core records the request and source identity but never fetches the source.","inputSchema":obj({"source_analysis_id":{"type":"string"},"reason":{"type":"string","enum":["old","unavailable","newer_version_known"]},"details":{"type":"string"},"known_source_version":{"type":"string"},"idempotency_key":{"type":"string"}},["source_analysis_id","reason"]),"annotations":{"readOnlyHint":False}},
    {"name":"investigation_complete","description":"Mark an investigation completed without rewriting its source analyses or claims.","inputSchema":obj({"investigation_id":{"type":"string"}},["investigation_id"]),"annotations":{"readOnlyHint":False}},
    {"name":"wiki_page_create","description":"Create a topic-oriented Decision Wiki page. Manual notes remain separate from generated revisions.","inputSchema":obj({"project_id":{"type":"string"},"topic":{"type":"string"},"title":{"type":"string"},"scope_id":{"type":"string"},"idempotency_key":{"type":"string"}},["project_id","topic","title"]),"annotations":{"readOnlyHint":False}},
    {"name":"wiki_note_set","description":"Replace the explicitly manual notes attached to a Wiki page without modifying generated revision content.","inputSchema":obj({"page_id":{"type":"string"},"manual_notes":{"type":"string"}},["page_id","manual_notes"]),"annotations":{"readOnlyHint":False}},
    ...
    {"name":"wiki_markdown_export","description":"Export a deterministic, bounded set of current Wiki pages as linked Markdown documents with stable page and revision metadata. SQLite remains authoritative.","inputSchema":obj({"project_id":{"type":"string"},"scope_id":{"type":"string"},"limit":{"type":"integer","minimum":1,"maximum":100},"offset":{"type":"integer","minimum":0}},["project_id"]),"annotations":{"readOnlyHint":True}},
...
CORE_TOOL_NAMES = {"context_bootstrap","project_resolve","session_start","session_end","record_event","read_events_since","event_poll","event_ack",
                   ...
                   "wiki_note_set","wiki_revision_generate","wiki_revision_transition","wiki_page_get","wiki_browse","wiki_revision_render","wiki_markdown_export","wiki_revision_lint"}
...
class MCPServer:
    ...
    def call(self, name: str, a: dict[str, Any]) -> Any:
        mapping = {
            ...
            "event_poll":self.store.poll_events, "event_ack":self.store.acknowledge_events,
            ...
            "investigation_create":self.store.create_investigation,"investigation_record_source":self.store.record_source_analysis,
            "investigation_get":self.store.get_investigation,"investigation_complete":self.store.complete_investigation,
            "source_reinspection_request":self.store.request_source_reinspection,

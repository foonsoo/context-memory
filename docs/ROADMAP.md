...
5. **Split persistence by bounded domain behind a stable facade — in progress.** Composed repositories now own bounded project/session/evidence, memory identity, investigation read-model, Wiki identity, checkpoint observation, and maintenance/audit queries using the facade's transaction connection. `context_memory.store.MemoryStore` remains the compatibility facade without mixin inheritance or repository-to-repository dependencies. Continue moving the remaining writes and large query assemblies until `store.py` owns orchestration and transaction boundaries rather than feature SQL.
6. **Decompose CLI and MCP declarations — in progress.** CLI parser construction is now separate from execution, preserving the frozen command and argument contract. Next replace the execution chain with an explicit command registry, then move MCP schema declarations into readable per-domain definitions. Preserve tool pagination, profiles, validation errors, and zero runtime dependencies.

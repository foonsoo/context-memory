...
5. **Split persistence by bounded domain behind a stable facade — in progress.** Composed repositories now own bounded project/session/evidence, memory lifecycle and embedding persistence, investigation read-model, Wiki identity, checkpoint observation, and maintenance/audit queries using the facade's transaction connection. `context_memory.store.MemoryStore` remains the compatibility facade without mixin inheritance or repository-to-repository dependencies. Continue moving the remaining retrieval assemblies, maintenance/import-export writes, and large queries until `store.py` owns orchestration and transaction boundaries rather than feature SQL.

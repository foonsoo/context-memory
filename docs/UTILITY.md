...
The Decision Brief regression is runnable with `PYTHONPATH=src python3 benchmarks/run_decision_evaluation.py`. Its frozen schema-v1 fixture covers changed decisions, conflicting evidence, rejected alternatives, outcome feedback, missing rationale, stale external sources, negative questions, lexical paraphrases, duplicate summaries, cross-project isolation, and a 120-distractor candidate set. Report schema v2 evaluates both FTS-only and default local-hash modes and records Recall@5, MRR@5, negative-query false-result rate, duplicate rate, current-decision accuracy, stale-decision leakage, unsupported-claim rate, source recovery, useful-history recall, compact JSON character size, and p50/p95 latency. This deterministic suite is an exit-gate regression for `decision-brief/v1`, not evidence that the product improves human decisions.

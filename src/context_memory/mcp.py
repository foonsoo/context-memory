...
TOOLS = [
    ...
    {"name": "record_event", "description": "Append immutable raw evidence. Success is returned only after durable DB commit.", "inputSchema": obj({"project_id":{"type":"string"},"kind":{"type":"string"},"content":{"type":"string"},"session_id":{"type":"string"},"scope_id":{"type":"string"},"source_uri":{"type":"string"},"metadata":{"type":"object"},"idempotency_key":{"type":"string"}}, ["project_id","kind","content"]), "annotations":{"readOnlyHint":False}},
    {"name": "memory_upsert", "description": "Propose, activate, or update a derived memory with source event provenance.", "inputSchema": obj({"project_id":{"type":"string"},"title":{"type":"string"},"content":{"type":"string"},"memory_type":{"type":"string","enum":["fact","decision","preference","constraint","procedure","summary","task","other"]},"status":{"type":"string","enum":["proposed","active","superseded","disputed","expired","rejected"]},"confidence":{"type":"number","minimum":0,"maximum":1},"importance":{"type":"number","minimum":0,"maximum":1},"scope_id":{"type":"string"},"source_event_ids":{"type":"array","items":{"type":"string"}},"valid_from":{"type":"string"},"valid_until":{"type":"string"},"tags":{"type":"array","items":{"type":"string"}},"memory_id":{"type":"string"},"idempotency_key":{"type":"string"}}, ["project_id","title","content"]), "annotations":{"readOnlyHint":False}},
    ...
    {"name": "memory_search", "description": "Rank memories using local FTS5 plus importance and confidence. Embeddings are not required.", "inputSchema": obj({"project_id":{"type":"string"},"query":{"type":"string"},"limit":{"type":"integer","minimum":1,"maximum":100},"statuses":{"type":"array","items":{"type":"string"}},"scope_id":{"type":"string"}}, ["project_id","query"]), "annotations":{"readOnlyHint":True}},
    {"name": "get_context", "description": "Select task-relevant active/disputed memories; requested budget is capped by the project policy (12,000 characters and 20 items by default).", "inputSchema": obj({"project_id":{"type":"string"},"query":{"type":"string"},"char_budget":{"type":"integer","minimum":0,"maximum":100000},"statuses":{"type":"array","items":{"type":"string"}},"scope_id":{"type":"string"}}, ["project_id","query"]), "annotations":{"readOnlyHint":True}},
...
class MCPServer:
    ...
    def call(self, name: str, a: dict[str, Any]) -> Any:
        mapping = {
            ...
            "memory_search": self.store.search, "get_context": self.store.get_context,

...
2. **Adopt PEP 8 as the repository convention — in progress.** Ruff 0.16.1 is pinned in the development-only extra with explicit `E`, `W`, `F`, `I`, and `N` rules, 79-character code, 72-character comments/docstrings, and deterministic formatting. CI runs both lint and format checks without adding a runtime dependency. The positive adoption list now covers `__init__`, audit anchor, backup crypto, CLI, contracts, embeddings, hooks, MCP, and Tasks; all pass without per-file ignores. The large `store.py` module has completed its behavior-preserving structural formatting and comment/docstring wrapping passes; its remaining long SQL and diagnostic strings must be wrapped before it joins the positive list. The list expands only after each remaining module is cleaned, keeping existing violations visible rather than hiding them behind a permanent ignore baseline.

...
TOOLS = [
    ...
    {"name": "get_context", "description": "Select local and global memories first, then search the whole database and aggregate project relevance/activity when local results are empty. Ambiguous matches are returned as project candidates instead of mixing memories.", "inputSchema": obj({"project_id":{"type":"string"},"query":{"type":"string"},"char_budget":{"type":"integer","minimum":0,"maximum":100000},"statuses":{"type":"array","items":{"type":"string"}},"scope_id":{"type":"string"},"event_cursor":{"type":"integer","minimum":0},"event_kinds":{"type":"array","items":{"type":"string"}},"event_limit":{"type":"integer","minimum":1,"maximum":1000},"event_char_budget":{"type":"integer","minimum":0,"maximum":4000},"discover_projects":{"type":"boolean"}}, ["project_id","query"]), "annotations":{"readOnlyHint":True}},

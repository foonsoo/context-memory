...
TOOLS = [
    ...
    {"name":"checkpoint_create","description":"Idempotently record an explicit interim or final recovery checkpoint without mutating Git, memories, or session lifecycle.","inputSchema":obj({"project_id":{"type":"string"},"mode":{"type":"string","enum":["interim","final"]},"reason":{"type":"string","enum":["context_budget","elapsed","material_change","completed","manual"]},"goal":{"type":"string"},"idempotency_key":{"type":"string"},"session_id":{"type":"string"},"scope_id":{"type":"string"},"completed":{"type":"array","items":{"type":"string"}},"next_step":{"type":"string"},"blockers":{"type":"array","items":{"type":"string"}},"source_event_cursor":{"type":"integer","minimum":0},"context_usage":{"type":"number","minimum":0,"maximum":1}},["project_id","mode","reason","goal","idempotency_key"]),"annotations":{"readOnlyHint":False}},
...
class MCPServer:
    ...
    def call(self, name: str, a: dict[str, Any]) -> Any:
        mapping = {
            ...
            "checkpoint_create":self.store.create_checkpoint, "read_events_since":self.store.read_events_since,

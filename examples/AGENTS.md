...
- At the start of a task, call `project_resolve` with the current workspace directory as an identity hint. Use the returned project and scope IDs for writes, then make a focused `get_context` request with a 4,000–8,000 character budget; retrieval can discover relevant registered projects when the hinted partition has no match. Treat disputed entries as warnings and inspect cited events with `get_source` before relying on consequential claims.

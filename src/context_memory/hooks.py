...
def checkpoint_from_hook(store: MemoryStore, project_id: str, session_id: str | None,
                         payload: dict, scope_id: str | None = None) -> dict | None:
    """Turn an optional client lifecycle signal into the portable checkpoint call.

    Hook payloads are deliberately treated as unverified recovery state. A client
    may explicitly request a final checkpoint only by supplying the same verified
    evidence and handoff fields required by ``MemoryStore.create_checkpoint``.
    """
    state = payload.get("context_memory") or {}
    if not isinstance(state, dict):
        raise ValueError("context_memory hook state must be an object")
    goal = str(state.get("goal") or payload.get("last_assistant_message") or "Resume client work")
    ...
    if mode == "final":
        return store.create_checkpoint(
            project_id, "final", state.get("reason", "completed"), goal,
    ...
    evaluation = store.evaluate_checkpoint(
        project_id, context_usage=state.get("context_usage"), session_id=session_id,
        repository_path=state.get("repository_path"), goal=goal,
        completed=completed, next_step=next_step, blockers=blockers,
    )
    if not evaluation["should_checkpoint"] and not state.get("force"):
        return None
    return store.create_checkpoint(
        project_id, "interim", evaluation.get("recommended_reason") or state.get("reason", "manual"),
        goal, state.get("idempotency_key") or evaluation["suggested_idempotency_key"],
        session_id=session_id, scope_id=scope_id, completed=completed,
        next_step=next_step, blockers=blockers,
        source_event_cursor=state.get("source_event_cursor"),
        context_usage=state.get("context_usage"),
        repository_path=state.get("repository_path"), test_results=state.get("test_results"),
    )
    ...
    try:
        ...
        if action == "start":
            ...
            if row:
                ...
                checkpoint = checkpoint_from_hook(store, project["id"], row["id"], payload, scope_id)

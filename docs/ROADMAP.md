...
P0 Decision Brief/evaluation, P1 Research-to-Decision provenance, and P2 topic Wiki revisions are implemented. The active work is **P2.5 non-neural retrieval quality and latency**; its evaluation baseline, query-path overhead pass, strict lexical candidate generation, calibrated negative-result gate, and bounded local-hash pass are complete. The next implementation step is measured local-hash recall improvement, followed by P3 lint/review. Neural embedding remains an optional evaluated adapter and is not part of the active implementation path because its measured latency cost did not justify making it the default.
...
6. **Improve local-hash recall without sacrificing precision.** Add frozen cases where FTS misses but human relevance judgments accept a result: paraphrases with little lexical overlap, partial wording, typographical variation, abbreviations, and Korean spacing/particle/expression changes. Evaluate separate word/character feature spaces, title/content/tag weights, corpus-derived IDF weighting, common-feature suppression, and larger hash dimensions. Accept a representation change only when it adds measured Recall@5/MRR over FTS-only and passes the negative-result and latency gates. If no configuration provides net value, remove local-hash from the default path while retaining explicit opt-in evaluation.
7. **Add Decision Brief reranking.** Rerank only the bounded top candidates using question intent, memory type/status, direct provenance, decision/rationale/constraint/outcome role, and penalties for unsupported, stale proposed, or repetitive handoff content. Keep general `memory_search` semantics separate and expose every added score component.
8. **Expand from decision seeds.** After retrieving a small set of current decision seeds, traverse at most one hop by default for supports/depends-on/supersession and shared investigation relations. Deduplicate expanded evidence and charge it to the existing item/character budgets; use a second hop only when explicitly requested or when a tested retrieval gap requires it.
9. **Reduce cross-project discovery work.** Generate project candidates from lexical/alias/identity evidence first, run bounded local-hash only for plausible projects, and retain the existing conservative confidence and ambiguity rules. Never trade ambiguous project mixing for higher recall.
10. **Re-run the complete matrix after every ranking change.** Accept a change only when it improves a named accuracy or latency failure without worsening stale leakage, unsupported claims, source recovery, ambiguity safety, or bounded context size beyond a documented tolerance.
